...
> 适用岗位：**AI Native 开发工程师（实习生）**
> 该项目主要用于展示作者的 **Android 原生客户端工程能力**，包括：复杂 UI 还原、网络/数据层设计、后台播放服务、协程与并发、本地持久化与全局状态管理——这些能力正是把「模型能力落地为端侧 AI 应用」的基础工程底座。
...
## Roadmap 与 AI Native 探索

当前项目完整地展现了客户端工程能力，在此基础上我会继续以 **AI Native** 的思路演进，让「音乐 App」从工具型应用升级为「以模型为核心的智能体」：

- [x] 网易云全功能客户端（发现 / 播放 / 社交 / 账号，约 2 万行 Kotlin）
- [ ] **接入 LLM，打造 AI 音乐助手**：自然语言点歌、歌单智能总结与转文案、根据心情/场景生成歌单
- [ ] **心动模式增强**：基于本地收听行为 + Embedding 做端侧轻量相似歌曲召回，替代纯服务端推荐
- [ ] **语音交互**：结合系统语音识别，实现「下一首」「单曲循环」「播放我收藏的歌单」等指令
- [ ] **数据驱动的个性化**：把 Room 中的播放/收藏行为作为用户画像，参与推荐与每日推荐生成
- [ ] 架构升级：迁移 Compose + 单 Activity 多模块，便于后续以模块化方式承载 AI 能力

我对 AI Native 的理解：**不是给传统 App 套一层聊天框，而是让模型、上下文与工具调用成为产品的默认交互内核**。本项目沉淀的网络层、状态管理、服务化与端侧存储能力，正是支撑智能体稳定运行的工程基础，我会把这些经验迁移到 AI 应用开发中。
